// Netlify serverless function (standard Node "handler" format).
...
export const handler = async (event, context) => {
  ...
  if (!OPENAI_API_KEY) {
    return {
      statusCode: 500,
      body: JSON.stringify({
        error: "OPENAI_API_KEY haijawekwa kwenye Netlify environment variables. Nenda Site configuration → Environment variables.",
      }),
    };
  }
